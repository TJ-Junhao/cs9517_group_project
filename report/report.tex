...
\author{
\IEEEauthorblockN{
Junhao Bai(z5563906), Changyu Chen(z5697373), Xiao Dong(z5571806), Jianing Liu(z5645882), Alyanna Zhang(z5574238)
}
\IEEEauthorblockA{
...
with the best model reaching a Plant IoU of 0.8359, surpassing the
...
IoU of 0.28 at the highest severity---a complete ranking reversal
...
\begin{table}[t]
\centering
\caption{Summary of Implemented Methods}\label{tab:methods}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{@{}ll>{\raggedright\arraybackslash}p{2.8cm}@{}}
\toprule
...
respectively.}\label{fig:deep_arch}
\end{figure}
...
decoder feature $x_1$ and passed through a residual conv block).}\label{fig:deep_modules}
\end{figure}
...
0.897. Both methods require no training data and run in under 30\,ms
per image.
...
Gaussian pairwise potentials, reaches Plant IoU 0.547---below ExG
despite taking 1.46\,s per image. GrabCut and Watershed produce weaker
...
\begin{table}[t]
\centering
\caption{Test-set performance of traditional segmentation methods.}\label{tab:trad}
\renewcommand{\arraystretch}{1.1}
...
\midrule
ExG              & 0.9032 & 0.7536 & 0.8595 & 0.029 \\
HSV thresholding & 0.8972 & 0.7083 & 0.8292 & 0.012 \\
DenseCRF         & 0.7519 & 0.5471 & 0.7072 & 1.460 \\
GrabCut          & 0.6816 & 0.4994 & 0.6661 & 3.569 \\
Watershed        & 0.4650 & 0.3758 & 0.5463 & 0.066 \\
K-means          & 0.5858 & 0.2978 & 0.4589 & 0.843 \\
Edge-based       & 0.5796 & 0.2614 & 0.4145 & 0.039 \\
\bottomrule
\end{tabular}
\end{table}
...
and GrabCut underperform at substantially higher inference cost.}\label{fig:trad_comparison}
\end{figure}

\subsection{Classical ML Baselines and Feature Effects}

The classical results show two different patterns between the two
models. Plant-soil separation can be achieved to a reasonable extent
using handcrafted colour information, but the quality remains limited
once the visual conditions become less ideal. Logistic Regression
benefits from richer colour features, with Plant IoU rising from 0.7012
(RGB) to 0.7138 (RGB+HSV); adding ExG on top brings no further change,
since HSV already captures the relevant chromatic information. Random
Forest, by contrast, already saturates on RGB alone (Plant IoU 0.6839),
and adding HSV or ExG yields no improvement. This likely reflects that
tree ensembles can internally reconstruct the nonlinear colour
interactions that HSV and ExG encode explicitly, making those
hand-designed features redundant once an RGB-based nonlinear classifier
is in place.

\begin{table}[t]
\centering
\caption{Test-set performance of classical methods under different
feature configurations.}\label{tab:classical}
\renewcommand{\arraystretch}{1.1}
...
\midrule
Logistic Reg. & RGB         & 0.8684 & 0.7012 & 0.8243 & 0.019 \\
Logistic Reg. & RGB+HSV     & 0.8776 & 0.7138 & 0.8330 & 0.132 \\
Logistic Reg. & RGB+HSV+ExG & 0.8776 & 0.7138 & 0.8330 & 0.232 \\
\midrule
Random Forest & RGB         & 0.8649 & 0.6839 & 0.8123 & 1.948 \\
Random Forest & RGB+HSV     & 0.8624 & 0.6813 & 0.8104 & 1.847 \\
Random Forest & RGB+HSV+ExG & 0.8634 & 0.6825 & 0.8113 & 1.969 \\
\bottomrule
\end{tabular}
\end{table}
...
ablation---performance is already saturated at RGB, with RGB+HSV and
RGB+HSV+ExG providing no further gain.}\label{fig:classical_comparison}
\end{figure}
...
with our strongest model reaching 0.8359---an improvement of 6.09
...
from U-Net to ResU-Net, with Plant IoU increasing by about 1.52
percentage points. By contrast, ASPP and attention gating produced
gains of 0.05 and 0.39 percentage points respectively. This suggests
...
IoU is about 0.9 percentage points. This gap is attributable to a
...
\begin{table*}[t]
\centering
\caption{Deep-model performance across train, validation, and test splits.}\label{tab:deep}
\renewcommand{\arraystretch}{1.1}
...
\textbf{Model} &
  ...
  \textbf{Inference (s)} \\
\midrule
U-Net (Baseline)     & 0.7659 & 0.8317 & 0.8163 & 0.8989 & 0.9062 & 0.9338 & 0.5760 \\
ResU-Net             & 0.8362 & 0.8424 & 0.8315 & 0.9080 & 0.9157 & 0.9405 & 0.6588 \\
ASPP-ResU-Net        & 0.8390 & 0.8397 & 0.8320 & 0.9083 & 0.9154 & 0.9404 & 0.7662 \\
AG-ASPP-ResU-Net     & \textbf{0.8501} & \textbf{0.8447} & \textbf{0.8359} & \textbf{0.9106} & \textbf{0.9177} & \textbf{0.9420} & 0.8491 \\
\midrule
...
gating deliver smaller additional gains.}\label{fig:deep_comparison}
\end{figure}
...
plant pixels rather than soil, consistent with boundary-level ambiguity.}\label{fig:training_cm}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Negative-result ablations.}\label{tab:ablation}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{1.9cm}>{\raggedright\arraybackslash}p{3.0cm}l@{}}
\toprule
...
about 0.8188, clearly below the standard ResU-Net result (0.8315).
...
falls from 0.836 to 0.789 at the highest severity, a decline that
leaves the model still functional. JPEG compression shows a steeper
trend, with Plant IoU dropping from 0.815 to 0.617, indicating that
...
drops to 0.007---a near-complete collapse. More importantly, this
...
sharply from 0.556 to 0.126; by noise level 5, all three residual-based
models collapse to essentially zero Plant IoU (ResU-Net 0.004,
ASPP-ResU-Net 0.005, AG-ASPP-ResU-Net 0.007). The plain U-Net, by
contrast, degrades much more gracefully and retains a Plant IoU of
0.281 at the same severity. On highly noisy inputs, the clean-set
...
0.734$\to$0.281), while AG-ASPP-ResU-Net collapses between levels~2
and~3, reaching 0.007 at level~5 despite leading on clean data.}\label{fig:noise_reversal}
\end{figure}

\begin{table}[t]
\centering
\caption{Summary of robustness behaviour under different corruptions
(AG-ASPP-ResU-Net; noise comparison includes U-Net).}\label{tab:robust}
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{1.4cm}>{\raggedright\arraybackslash}p{3.2cm}@{}}
\toprule
\textbf{Corruption} & \textbf{Robustness} & \textbf{Key observation} \\
\midrule
Affine rotation  & Very strong & Almost no drop across levels \\
Gaussian blur    & Strong      & Plant IoU 0.836$\to$0.789 at highest severity \\
JPEG compression & Medium      & Plant IoU 0.815$\to$0.617; gradual boundary loss \\
Brightness shift & Uneven      & Underexposure $\gg$ overexposure; non-monotonic \\
Gaussian noise   & Weak        & All residual models $\le$0.007 at level~5;
                                 U-Net retains 0.281 (ranking reversed) \\
\bottomrule
\end{tabular}
\end{table}
...
fragmented predictions relative to the ground-truth mask.}\label{fig:failure}
\end{figure}
...
incidental. The 0.9 percentage-point drop in Plant IoU from validation
...
remains relatively more stable and retains a Plant IoU of 0.281 at the
